...
T=open(os.path.join(HERE,"prompts/missing-demo.txt")).read()
def call(text):
    prompt=T+text+"\n--- END ---\n"
    for _ in range(3):
        ...
        try:
            ...
            if o.get("subtype")=="success" and not o.get("is_error"):
                ...
                if m: return json.loads(m.group(0)).get("missing_demo",[])
...
def run(fix,K=5):
    ...
    print(f"[{tag:8}] {fix:26} expect_flag={want!s:5} flagged={flagged} correct={correct}/{len(res)}")
    return correct,len(res),want
if __name__=="__main__":
    fixtures=sys.argv[1:] or ["missing-demo-cli","missing-demo-app",
        "missing-demo-has-gif","missing-demo-library","missing-demo-reference"]
    tot=ok=0
    for f in fixtures:
